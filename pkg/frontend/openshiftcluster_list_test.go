		...
		t.Run(tt.name, func(t *testing.T) {
			for name, listPrefix := range map[string]string{
				...
				t.Run(name, func(t *testing.T) {
					...
					f, err := NewFrontend(ctx, ti.audit, ti.log, ti.env, ti.asyncOperationsDatabase, ti.openShiftClustersDatabase, ti.subscriptionsDatabase, api.APIs, &noop.Noop{}, aead, nil, nil, nil, func(log *logrus.Entry, dialer proxy.Dialer, m metrics.Emitter) clusterdata.OpenShiftClusterEnricher {
						return ti.enricher
					})
					if err != nil {
						t.Fatal(err)
